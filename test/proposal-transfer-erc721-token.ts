...
import { IFGovernor, wSMR, IFTimelock, MockErc721 } from "../typechain-types";
import deployIFGovernor from "../deploy/if-governor";
import deployIFVotesToken from "../deploy/wSMR-token";
import deployMockErc721 from "../deploy/mock-erc721-token";
...
describe("IF governance test of proposal creation for transferring ERC721 tokens", () => {
  let IFGovernorContract: IFGovernor;
  let IFVotesTokenContract: wSMR;
  let MockErc721Contract: MockErc721;
  let MockErc721ContractAddress: string;
  let MockErc721TokenId: number;
  ...
  const RECIPIENT_ERC721 = ethers.Wallet.createRandom().address;
  const RECIPIENT_ERC721_AMOUNT = 1; // 1 erc721 tokens
  ...
  it("Deploy governance contracts", async () => {
    ...
    MockErc721Contract = await deployMockErc721();
    MockErc721ContractAddress = await MockErc721Contract.getAddress();
    await MockErc721Contract.mintNewItem();
    MockErc721TokenId = 1;
  ...
  it("Any attempt to create proposal from not whitelisted address will fail.", async () => {
    ...
    encodedFunctionCall = IFTimelockContract.interface.encodeFunctionData(
      "transferNFT",
      [
        MockErc721ContractAddress,
        RECIPIENT_ERC721,
        MockErc721TokenId,
        RECIPIENT_ERC721_AMOUNT,
      ]
    );
  ...
  it("Create proposal to transfer ERC721 tokens to the specified RECIPIENT_ERC721", async () => {
  ...
  it("Queue & Execute the proposal", async () => {
    ...
    // Fund ERC721 tokens for the Timelock
    const timelockAddress = await (IFTimelockContract as any).getAddress();

    await MockErc721Contract.safeTransferFrom(
      signer.address,
      timelockAddress,
      MockErc721TokenId
    );
    expect(await MockErc721Contract.balanceOf(timelockAddress)).to.equal(
      RECIPIENT_ERC721_AMOUNT
    );
    expect(await MockErc721Contract.balanceOf(RECIPIENT_ERC721)).to.equal(0);
    //////
    ...
    expect(await MockErc721Contract.balanceOf(timelockAddress)).to.equal(0);
    expect(await MockErc721Contract.balanceOf(RECIPIENT_ERC721)).to.equal(
      RECIPIENT_ERC721_AMOUNT
    );
  });
